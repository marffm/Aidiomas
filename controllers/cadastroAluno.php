...
class cadastroAluno extends Controller {
    ...
    function index() {
        ...
        if (isset($_SESSION['error_insertAluno'])) {
            $error = 'Aluno já existe no sistema.';
            $_SESSION['error_insertAluno'] = null;
        }
    ...
    function checkAluno() {
        $validate = $this->cadastroAlunoModel->checkAluno();
        
        if ($validate == false) {
            $_SESSION['error_insertAluno'] = true;
        } else {
            $this->cadastroAlunoModel->insertAluno();
        }
        return $validate;
    }
